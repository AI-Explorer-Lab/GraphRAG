...
from lineage_graphrag.llm.prompts import build_decomposition_prompt
...
class LineageQuestionDecomposer:
    ...
    @classmethod
    def from_config(cls, cfg: AppConfig) -> "LineageQuestionDecomposer":
    ...
    def decompose(self, question: str) -> dict[str, object]:
        ...
        if llm_sub_questions:
            return {
                ...
                # Lineage schema is fixed (has/transitions), so this hint is optional.
                "involved_types": {},
            }
    ...
    def _fallback(self, question: str) -> list[str]:
        text = question.strip()
        if not text:
            return ["Explain the lineage graph and key upstream/downstream dependencies."]
